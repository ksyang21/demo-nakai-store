            ...
            <div class="flex md:flex-col flex-col space-y-7 justify-center">
                  ...
                  <div v-for="(size, i) in sizes" :key="i" class="">
                    <div
                      @click="
                        ...
                        variants_id = product.variants[i].id;
                      "
                      :class="
                        currentSize === size
                      ...
                      class="border-2 rounded-md cursor-pointer flex justify-center py-3 px-5"
                    >
              ...
              <div>
                <div class="flex flex-col space-y-3">
                  <div>
                    <span class="text-gray-700 text-2xl font-san"
                      >&euro; {{ currentPrice }}</span
                    >
                  </div>

                  <div
                    @click="addProduct()"
                    class="bg-gray-900 cursor-pointer text-white w-full text-sm font-semibold py-3 flex justify-center cursor pointer hover:bg-white hover:border hover:border-gray-900 hover:text-black"
                  >
                    ADD TO CART
                  </div>
                </div>
...
export default {
  ...
  beforeMount() {
    ...
    axios
      .get(`${import.meta.env.VITE_baseUrl}/store/products/${this.productId}`)
      .then((productData) => {
